...
if (wrapper) {
  ...
  requireText(
    ...
    `${wrapperPath}: recovery-remount stenger ikke server-first-gaten eksplisitt i appmodus.`
  );
  requireText(
    rehydrateBlock,
    'setServerCacheError("")',
...
console.log(
  "✅ Expo ProffDok Sales lazy-loading check OK – oversikten bruker lett projeksjon, valgt sak hydreres komplett før redigering, recovery-remount stenger server-gaten før Core remountes, summary kan ikke lagres tilbake, nettfeil blokkerer ufullstendig editor og 42F recovery/server-first er bevart"
);
