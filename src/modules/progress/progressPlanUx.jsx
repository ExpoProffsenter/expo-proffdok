...
// Profesjonell fremdriftsplan som et eget arbeidslag over eksisterende prosjekt-UI.
// Intern plan er redigerbar. UE ser planen read-only. Kunde ser den kun når bedriften
// eksplisitt har aktivert kundedeling. Avansert drag/drop kommer senere.
...
function ProgressPlanWorkspace({ mode, identity, requestRef, projectId: portalProjectId, role, onDirtyChange }) {
  ...
  const removeActivity = (activityId) => {
    if (!window.confirm('Fjerne denne aktiviteten fra fremdriftsplanen?')) return;
    markPlan((prev) => ({
      ...prev,
  ...
  const importOffer = async () => {
    ...
    try {
      ...
      const additions = imported
        ...
        .filter((activity) => {
          const key = clean(activity.sourceMainPostId || activity.title).toLowerCase();
    ...
    return (
      <div className="progress-shell">
        <div className="progress-card progress-resolver">
          ...
          <p>
            Prosjektnavnet er ikke entydig nok til at Expo ProffDok kan koble planen automatisk.
            Velg prosjektet én gang for denne visningen. Dette hindrer at en plan lagres på feil prosjekt.
          </p>
    ...
    return (
      <div className="progress-shell">
        <div className="progress-card">
          ...
          <p>Fremdriftsplanen er ikke delt med kunde på dette prosjektet.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="progress-shell">
      <section className="progress-hero">
        <div>
          ...
          <p>
            ...
            {projectMeta?.address ? ` · ${projectMeta.address}` : ''}
          </p>
        </div>
      ...
      {projectMeta?.locked && !portalMode ? (
        <div className="progress-note">🔒 Prosjektet er låst. Fremdriftsplanen vises som historikk og kan ikke redigeres.</div>
      ) : null}
      {mode === 'underleverandor' ? (
        <div className="progress-note">UE-visning: planen kan leses, men endres ikke her.</div>
      ) : null}
      {mode === 'customer' ? (
        <div className="progress-note">Kundevisning: dette er bedriftens delte fremdriftsplan.</div>
      ) : null}
      {error ? <div className="progress-error">{error}</div> : null}
        ...
        {!activities.length ? (
          ...
          <div className="progress-board-scroll">
            <div className="progress-board" style={{ '--progress-days': days.length }}>
              ...
              {days.map((day) => (
                <div key={isoDate(day)} className={`progress-grid-head ${isoDate(day) === isoDate(new Date()) ? 'today' : ''}`}>
                  {formatDay(day)}
                </div>
              ))}
                  ...
                  <React.Fragment key={activity.id}>
                    <div className="progress-activity-cell progress-sticky-left">
                      <div className="progress-activity-title-row">
                        <strong>{activity.title || 'Aktivitet'}</strong>
...
function modeFromLocation() {
  ...
  const role = clean(params.get('role') || params.get('access') || '').toLowerCase();
...
function currentSalesRef() {
  try {
    return clean(window.sessionStorage.getItem(SALES_REF_KEY) || '');
  } catch {
    return '';
  }
}
...
function findNav(mode) {
  ...
  return navs.find((nav) => {
    const labels = Array.from(nav.querySelectorAll('button')).map((button) => clean(button.textContent));
    return labels.includes('Prosjektoversikt') && labels.includes('Sjekklister');
  }) || null;
}
...
function closeProgressPlan({ force = false } = {}) {
  ...
  if (!force && currentDirty && !window.confirm('Du har ulagrede endringer i fremdriftsplanen. Gå videre uten å lagre?')) {
    return false;
  }
  active = false;
  currentDirty = false;
...
function injectDesktopNav(mode) {
  ...
  let button = nav.querySelector(`button[${NAV_ATTR}="1"]`);
  if (!show) {
    button?.remove();
  ...
  if (!button) {
    button = makeProgressButton();
    const nativeButtons = Array.from(nav.querySelectorAll('button'));
    const targetLabel = mode === 'internal' ? 'Prosjektoversikt' : mode === 'underleverandor' ? 'Prosjektinformasjon' : 'Oversikt';
    const target = nativeButtons.find((candidate) => clean(candidate.textContent) === targetLabel);
    if (target?.nextSibling) nav.insertBefore(button, target.nextSibling);
    else nav.append(button);
  }
  button.classList.toggle('on', active);
  if (active) {
    Array.from(nav.querySelectorAll('button.on')).forEach((candidate) => {
      if (candidate !== button) candidate.classList.remove('on');
    });
  }
}
