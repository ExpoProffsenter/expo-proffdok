...
function saveLocal(projectId, rows) {
  window.localStorage.setItem(safeKey(projectId), JSON.stringify(meaningfulParticipants(rows).map((row) => ({ ...row, local: true }))));
}
...
function focusParticipant(id) {
  window.setTimeout(() => {
    ...
    row?.querySelector('input[type="text"],input[type="email"],input[type="tel"]')?.focus();
  }, 30);
}

function ParticipantRow({ row, onChange, onRemove, onEnter }) {
  ...
  return (
    <div className="project-participant-row" data-participant-id={row.id}>
      <label>Navn<input type="text" value={row.name} onChange={(e) => set('name', e.target.value)} onKeyDown={enter} /></label>
      <label>Firma<input type="text" value={row.company} onChange={(e) => set('company', e.target.value)} onKeyDown={enter} /></label>
      <label>Rolle<input type="text" value={row.role} onChange={(e) => set('role', e.target.value)} onKeyDown={enter} placeholder="F.eks. prosjektleder" /></label>
      <label>E-post<input type="email" value={row.email} onChange={(e) => set('email', e.target.value)} onKeyDown={enter} /></label>
      <label>Telefon<input type="tel" value={row.phone} onChange={(e) => set('phone', e.target.value)} onKeyDown={enter} /></label>
      <div>
...
function ProjectParticipantsPanel({ projectId }) {
  ...
  useEffect(() => {
    window.dispatchEvent(new CustomEvent(DIRTY_EVENT, { detail: { projectId, dirty } }));
    return () => window.dispatchEvent(new CustomEvent(DIRTY_EVENT, { detail: { projectId, dirty: false } }));
  }, [dirty, projectId]);
...
function ensureParticipantsPanel() {
  ...
  if (!host || host.parentElement !== section || panelProjectId !== projectId) {
    if (panelRoot) unmountParticipantsPanel();
