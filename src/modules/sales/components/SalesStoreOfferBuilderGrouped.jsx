...
// Enter i prisfelt følger samme arbeidsmønster som ordinær tilbudsbygger:
// neste post i samme gruppe åpnes, ellers opprettes en ny post og får fokus.
...
function normalizeLegacySection(section = {}) {
  const title = cleanText(section.storeTextTitle);
  const body = cleanText(section.storeTextBody);

  // Tidlig 39B.2C hadde «Nytt avsnitt» som faktisk verdi og et umerket tekstfelt
  // under. Dersom brukeren skrev f.eks. «Varmepumpe» der, var det åpenbart ment
  // som avsnittsnavnet. Flytt det én gang til den nye, tydelige modellen.
...
function createStoreOption(product = null, type = "addition", installation = null) {
  ...
  const option = {
    ...
    storeInstallationUnitPriceInclVat:
      installation?.storeUnitPriceInclVat || "",
    title: "",
    description:
      alternative
        ? ""
        : product?.description
          ? `Tillegg / oppgradering til ${product.description}`
          : "",
    nobbNumber: "",
    supplierProductNumber: "",
    internalProductNumber: "",
  ...
  if (alternative && installation) {
    ...
    option.storeInstallationPriceInclVat = storeNumber(
      storeGrossTotal(installation)
    );
  }
...
function hasStoreProductDraftContent(item = {}) {
  return Boolean(
    cleanText(item.description) ||
      cleanText(item.storeUnitPriceInclVat) ||
      cleanText(item.supplierProductNumber) ||
      cleanText(item.internalProductNumber) ||
      cleanText(item.nobbNumber) ||
      cleanText(item.productUrl) ||
      cleanText(item.storeCatalogItemId) ||
      item.imageDataUrl ||
      item.attachmentFile?.url
  );
}

function hasStoreOptionDraftContent(item = {}) {
  return Boolean(
...
function normalizePricedItem(item = {}, patch = {}) {
  ...
  if (
    Object.prototype.hasOwnProperty.call(patch, "nobbNumber") &&
    (!cleanText(next.productUrl) || next.storeAutoProductUrl)
  ) {
  ...
  if (
    Object.prototype.hasOwnProperty.call(patch, "productUrl") &&
    patch.storeAutoProductUrl === false
  ) {
...
function composeLines({ products, installations, sections, metaLine, otherLines }) {
  const sectionIds = new Set(
    sections.map((section) => String(section.id || section.storeSectionId || ""))
  );
  const result = [];

  sections.forEach((rawSection) => {
    const section = normalizeLegacySection(rawSection);
    result.push({
      ...section,
      storeSectionMode: "group",
    ...
    products
      .filter(
        (product) =>
          String(product.storeSectionId || "") ===
          String(section.id || section.storeSectionId || "")
      )
  ...
  products
    .filter(
      (product) =>
        !product.storeSectionId ||
        !sectionIds.has(String(product.storeSectionId))
    )
    .forEach((product) => result.push(product));
        ...
        <div className="store-link-field">
          ...
          {nobbUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              title="Åpne varen direkte i NOBB"
            >
...
function PriceFields({
  ...
  onPatch,
  priceLabel,
  totalLabel = "Linjesum inkl. mva.",
  onPriceEnter,
}) {
  const unit = String(item.unit || "").toLowerCase();
  const resolvedPriceLabel =
    priceLabel ||
    (unit.startsWith("time")
      ? "Pris pr. time inkl. mva."
      : "Pris pr. enhet inkl. mva.");
  const grossUnit = storeGrossUnitPrice(item);
  const grossTotal = storeGrossTotal(item);
  const hasUnitPrice = cleanText(item.storeUnitPriceInclVat) !== "";
  ...
  return (
    <div className="store-price-grid">
      ...
      <label className="sales-field">
        ...
        />
        {hasUnitPrice ? (
          <small className="store-price-net">
            {formatNok(netFromGross(grossUnit))} eks. mva.
          </small>
        ) : null}
      </label>
      ...
      {item.imageDataUrl ? (
        <div className="store-file-chip">
          <img
            ...
            alt={item.imageName || "Produktbilde"}
          />
          <span>{item.imageName || "Produktbilde"}</span>
          <button type="button" onClick={onRemoveImage} aria-label="Fjern bilde">
            ×
          </button>
        </div>
      ) : null}
...
function CollapsibleHeader({
  ...
  return (
    <div
      className={`store-group-heading ${accent ? "is-accent" : ""}`}
      onClick={onToggle}
      role="button"
      tabIndex={0}
      onKeyDown={(event) => {
    ...
    >
      ...
      <div className="store-group-heading-actions">
        <span>{open ? "Lukk" : "Åpne"}</span>
        {onRemove ? (
          <button
            type="button"
...
function InstallationEditor({ item, open, onToggle, onPatch, onRemove }) {
  return (
    <article className="store-nested-card store-installation-card">
      <CollapsibleHeader
        title={item.description || "Montering"}
        summary={`${item.quantity || 1} ${item.unit || "timer"} · ${formatNok(
          storeGrossTotal(item)
        )}`}
        open={open}
        onToggle={onToggle}
        onRemove={onRemove}
      />
      {open ? (
        <div className="store-nested-body">
          <label className="sales-field">
            <span>Beskrivelse *</span>
            <input
              value={item.description || ""}
              onChange={(event) => onPatch({ description: event.target.value })}
              placeholder="F.eks. Montering av varen"
            />
          </label>
          <PriceFields
            item={item}
            onPatch={onPatch}
            totalLabel="Monteringssum inkl. mva."
          />
        </div>
      ) : null}
    </article>
...
function OptionEditor({
  ...
  const replacement = [...allProducts, ...installations].find(
  ...
  const parentInstallations = installations.filter(
    (line) =>
      String(line.storeParentProductId || "") === String(product?.id || "")
  );
  const primaryInstallation = parentInstallations[0] || null;
  const breakdown = alternative
    ? getStoreAlternativeBreakdown(option, [...allProducts, ...installations])
    : null;
  ...
  function setInstallationMode(mode) {
    ...
    if (mode === "none") {
      onPatch({
        storeInstallationMode: mode,
        storeInstallationReplacementLineId: primaryInstallation.id,
        storeInstallationPriceInclVat: "0",
      });
      return;
    }
    ...
    if (mode === "custom") {
      const quantity =
        option.storeInstallationQuantity || primaryInstallation.quantity || "1";
      const unitPrice =
        option.storeInstallationUnitPriceInclVat ||
        primaryInstallation.storeUnitPriceInclVat ||
        "";
      const total =
        parseStoreNumber(quantity, 1) * parseStoreNumber(unitPrice, 0);
      onPatch({
        storeInstallationMode: mode,
        storeInstallationReplacementLineId: primaryInstallation.id,
        storeInstallationQuantity: quantity,
        storeInstallationUnit:
          option.storeInstallationUnit || primaryInstallation.unit || "timer",
        storeInstallationUnitPriceInclVat: unitPrice,
        storeInstallationPriceInclVat: storeNumber(total),
      });
      return;
    }

    onPatch({
      storeInstallationMode: "same",
      storeInstallationReplacementLineId: primaryInstallation.id,
      storeInstallationPriceInclVat: storeNumber(
        storeGrossTotal(primaryInstallation)
      ),
    });
  }

  function patchCustomInstallation(patch) {
    const quantity = Object.prototype.hasOwnProperty.call(
      patch,
      "storeInstallationQuantity"
    )
      ? patch.storeInstallationQuantity
      : option.storeInstallationQuantity || primaryInstallation?.quantity || "1";
    const unitPrice = Object.prototype.hasOwnProperty.call(
      patch,
      "storeInstallationUnitPriceInclVat"
    )
      ? patch.storeInstallationUnitPriceInclVat
      : option.storeInstallationUnitPriceInclVat ||
        primaryInstallation?.storeUnitPriceInclVat ||
        "";
    const total =
      ...
      {open ? (
        <div className="store-nested-body">
            ...
            {alternative ? (
              <label className="sales-field">
                ...
                <select
                  ...
                  onChange={(event) => {
                    const target = [...allProducts, ...installations].find(
                      (line) => String(line.id) === event.target.value
                    );
                    onPatch({
                      replacementLineId: event.target.value,
                      replacementLineDescription: target?.description || "",
                      storeParentProductId:
                        target?.mainPostId === PRODUCT_POST.id
                          ? target.id
                          : option.storeParentProductId || "",
                    });
                  }}
                >
                  <option value="">Velg vare eller montering</option>
                  {allProducts.map((target) => (
                    <option key={target.id} value={target.id}>
                      Vare: {target.description || "Uten navn"}
                    </option>
                  ))}
                  {installations.map((target) => (
                    <option key={target.id} value={target.id}>
                      Montering: {target.description || "Uten navn"}
                    </option>
                  ))}
                </select>
              </label>
          ...
          {!replacesInstallation
            ? renderCatalogLookup?.({ kind: "option", item: option, onPatch })
            : null}
          ...
          {replacesInstallation ? (
            <>
              <label className="sales-field">
                <span>Beskrivelse *</span>
                <input
                  data-store-option-title={option.id}
                  value={option.title || ""}
                  onChange={(event) => onPatch({ title: event.target.value })}
                  placeholder="Ny monteringsløsning"
                />
              </label>
              <PriceFields
                item={option}
                onPatch={onPatch}
                onPriceEnter={onPriceEnter}
                priceLabel="Ny monteringspris inkl. mva."
                totalLabel="Ny monteringssum inkl. mva."
              />
            </>
          ) : (
            <>
              <ProductIdentityFields
                item={option}
                onPatch={onPatch}
                titleLabel={alternative ? "Alternativ vare *" : "Opsjonsvare *"}
                titleField="title"
                titleInputProps={{ "data-store-option-title": option.id }}
              />
              <PriceFields
                item={option}
                onPatch={onPatch}
                onPriceEnter={onPriceEnter}
                priceLabel={
                  alternative ? "Pris på alternativ vare inkl. mva." : undefined
                }
                totalLabel={
                  alternative
                    ? "Alternativ varepris inkl. mva."
                    : "Opsjonssum inkl. mva."
                }
              />
            </>
          ...
          {alternative && !replacesInstallation ? (
            <div className="store-option-installation-box">
              <strong>Montering ved alternativet</strong>
              {primaryInstallation ? (
                <>
                  <span>
                    Grunnvaren har {primaryInstallation.description || "montering"} på{" "}
                    {formatNok(storeGrossTotal(primaryInstallation))} inkl. mva.
                  </span>
                  <div className="store-choice-row">
                    <button
                      type="button"
                      className={
                        option.storeInstallationMode === "same" ||
                        (!option.storeInstallationMode &&
                          option.storeInstallationReplacementLineId)
                          ? "is-selected"
                          : ""
                      }
                      onClick={() => setInstallationMode("same")}
                    >
                      Samme montering
                    </button>
                    <button
                      type="button"
                      className={
                        option.storeInstallationMode === "custom"
                          ? "is-selected"
                          : ""
                      }
                      onClick={() => setInstallationMode("custom")}
                    >
                      Endre montering
                    </button>
                    <button
                      type="button"
                      className={
                        option.storeInstallationMode === "none"
                          ? "is-selected"
                          : ""
                      }
                      onClick={() => setInstallationMode("none")}
                    >
                      Ingen montering
                    </button>
                  </div>
                  ...
                  {option.storeInstallationMode === "custom" ? (
                    <div className="store-installation-custom-grid">
                      <label className="sales-field">
                        <span>Antall</span>
                        <input
                          value={
                            option.storeInstallationQuantity ||
                            primaryInstallation.quantity ||
                            "1"
                          }
                          inputMode="decimal"
                          onChange={(event) =>
                            patchCustomInstallation({
                              storeInstallationQuantity: event.target.value,
                            })
                          }
                        />
                      </label>
                      <label className="sales-field">
                        ...
                        <input
                          value={
                            option.storeInstallationUnit ||
                            primaryInstallation.unit ||
                            "timer"
                          }
                          onChange={(event) =>
                            onPatch({ storeInstallationUnit: event.target.value })
                          }
                        />
                      </label>
                      <label className="sales-field">
                        <span>Pris pr. time/enhet inkl. mva.</span>
                        <input
                          value={
                            option.storeInstallationUnitPriceInclVat ||
                            primaryInstallation.storeUnitPriceInclVat ||
                      ...
                      <div className="store-line-total">
                        <span>Monteringssum inkl. mva.</span>
                        <strong>
                          {formatNok(
                            parseStoreNumber(
                              option.storeInstallationPriceInclVat,
                              0
                            )
                          )}
                        </strong>
                ...
                <span>
                  Ingen montering er knyttet til denne varen. Legg til montering på
                  hovedvaren først dersom alternativet skal påvirke montering.
                </span>
              )}
            </div>
          ) : null}
          ...
          {alternative && breakdown ? (
            <div className="store-alt-summary">
              <div>
                <span>Alternativ vare</span>
                <strong>{formatNok(breakdown.newItemTotal)} inkl. mva.</strong>
              </div>
              {breakdown.hasInstallationOverride ? (
                <div>
                  <span>Montering med alternativet</span>
                  <strong>
                    {formatNok(breakdown.newInstallationTotal)} inkl. mva.
                  </strong>
                </div>
              ) : null}
              <div className="store-alt-summary-delta">
                <span>
                  {breakdown.totalDelta < 0
                    ? "Kunden får fratrekk"
                    : breakdown.totalDelta > 0
                      ? "Tillegg ved valg"
                      : "Prisendring"}
                ...
                <strong>
                  {breakdown.totalDelta < 0
                    ? formatNok(Math.abs(breakdown.totalDelta))
                    : formatStoreDelta(breakdown.totalDelta)}{" "}
                  inkl. mva.
                </strong>
              </div>
            </div>
    ...
    <section className="store-builder-section">
      <div className="store-section-head">
        <div>
          <h2>Avsender</h2>
          <p>
            Bademiljø Expo er standard. Valgt logo og saksbehandler låses med
            tilbudsversjonen.
          </p>
        </div>
      </div>
        ...
        {STORE_OFFER_BRANDS.map((brand) => (
          <label
            className={`store-brand-card ${
              brand.key === brandKey ? "is-selected" : ""
            }`}
            key={brand.key}
          >
            <input
...
function focusStoreProductTitle(lineId) {
  if (typeof window === "undefined" || typeof document === "undefined") return;
  window.setTimeout(() => {
    const field = document.querySelector(
      `[data-store-product-title="${lineId}"]`
    );
...
function focusStoreOptionTitle(optionId) {
  if (typeof window === "undefined" || typeof document === "undefined") return;
  window.setTimeout(() => {
    const field = document.querySelector(
      `[data-store-option-title="${optionId}"]`
    );
    if (field instanceof HTMLInputElement) {
      field.focus();
      field.select?.();
...
export default function SalesStoreOfferBuilderGrouped(props) {
  ...
  const previousOpenProductIdRef = useRef("");
  const previousOpenNestedIdRef = useRef("");
  ...
  const sections = lines.filter(isSectionLine).map(normalizeLegacySection);
  const otherLines = lines.filter(
    (line) =>
      !line?.__storeOfferMeta &&
      !isProductLine(line) &&
      !isInstallationLine(line) &&
      !isSectionLine(line)
  );

  const currentBrand = getStoreOfferBrand(
    metaLine?.brandKey || DEFAULT_STORE_OFFER_BRAND.key
  );
  const signatureName = String(
    metaLine?.signatureName ||
  ...
  const standaloneInstallations = installationLines.filter(
    (line) =>
      !line.storeParentProductId ||
      !productLines.some(
        (product) => String(product.id) === String(line.storeParentProductId)
      )
  );
  const standaloneOptions = options.filter(
    (option) =>
      ...
      !productLines.some(
        (product) =>
          String(product.id) === String(option.replacementLineId || "")
      )
  );
  ...
  useEffect(() => {
    ...
    let nextLines = lines.map((line) => {
      ...
      if (isSectionLine(line)) return normalizeLegacySection(line);

      let next = { ...line };
      if (next.nobbNumber && !cleanText(next.productUrl)) {
        next.productUrl = buildNobbItemUrl(next.nobbNumber);
        next.storeAutoProductUrl = true;
      }
      if (cleanText(next.storeUnitPriceInclVat)) {
        next.amount = canonicalNetUnit(next);
      }
      return next;
    });
  ...
  useEffect(() => {
    if (!previewOpen || typeof window === "undefined") return undefined;
  ...
  const baseGrossTotal = [...productLines, ...installationLines].reduce(
    (sum, item) => sum + storeGrossTotal(item),
    0
  );

  function applyGroups(
    nextProducts = productLines,
    nextInstallations = installationLines,
    nextSections = sections
  ) {
    const nextLines = composeLines({
  ...
  useEffect(() => {
    const previousId = previousOpenProductIdRef.current;
    previousOpenProductIdRef.current = openProductId;
    if (!previousId || previousId === openProductId) return;

    const previousProduct = productLines.find(
      (product) => String(product.id) === String(previousId)
    );
    if (!previousProduct || hasStoreProductDraftContent(previousProduct)) return;

    const hasChildren =
      installationLines.some(
        (line) =>
          String(line.storeParentProductId || "") === String(previousProduct.id)
      ) ||
      options.some(
        (option) =>
          String(option.storeParentProductId || "") ===
            String(previousProduct.id) ||
          String(option.replacementLineId || "") === String(previousProduct.id)
      );
    if (hasChildren) return;

    applyGroups(
      productLines.filter((product) => product.id !== previousProduct.id),
      installationLines,
      sections
    );
  ...
  useEffect(() => {
    const previousId = previousOpenNestedIdRef.current;
    previousOpenNestedIdRef.current = openNestedId;
    if (!previousId || previousId === openNestedId) return;

    const previousOption = options.find(
      (option) => String(option.id) === String(previousId)
    );
    if (!previousOption || hasStoreOptionDraftContent(previousOption)) return;

    updateOfferForm(
      "options",
      options.filter((option) => option.id !== previousOption.id)
    );
  }, [openNestedId]); // eslint-disable-line react-hooks/exhaustive-deps

  function patchProduct(id, patch) {
    applyGroups(
      productLines.map((item) =>
        item.id === id ? normalizePricedItem(item, patch) : item
      )
    );
  }
  ...
  function patchSection(id, patch) {
    applyGroups(
      ...
      sections.map((item) => {
        ...
        return {
          ...next,
          description: sectionDescription(next),
          productUrl: SECTION_MARKER,
          amount: "0",
          quantity: "0",
          unit: "",
  ...
  function patchStoreMeta(patch) {
    ...
    updateOfferForm(
      ...
      composeLines({
        ...
        installations: installationLines,
        sections,
        metaLine: nextMeta,
        otherLines,
      })
  ...
  function removeSection(id) {
    const nextProducts = productLines.map((product) =>
      String(product.storeSectionId || "") === String(id)
        ? { ...product, storeSectionId: "" }
        : product
    );
  ...
  function addProduct(sectionId = activeSectionId) {
    const line = createStoreLine(PRODUCT_POST, {
    ...
    focusStoreProductTitle(line.id);
    return line;
  }

  function removeProduct(id) {
    const nextInstallations = installationLines.filter(
      (line) => String(line.storeParentProductId || "") !== String(id)
    );
    const nextOptions = options.filter(
      (option) =>
        String(option.storeParentProductId || "") !== String(id) &&
        String(option.replacementLineId || "") !== String(id)
    );
    updateOfferForm("options", nextOptions);
    applyGroups(
      productLines.filter((line) => line.id !== id),
      nextInstallations
    );
  ...
  function addInstallation(product = null) {
    const line = createStoreLine(INSTALLATION_POST, {
      storeParentProductId: product?.id || "",
      description: product?.description
        ? `Montering av ${product.description}`
        : "Kun montering",
      unit: "timer",
    });
  ...
  function removeInstallation(id) {
    const remainingLines = lines.filter((line) => line.id !== id);
    const nextOptions = options.map((option) =>
      String(option.storeInstallationReplacementLineId || "") === String(id)
        ? recalculateStoreOption(
            {
              ...option,
              storeInstallationReplacementLineId: "",
    ...
    applyGroups(
      productLines,
      installationLines.filter((line) => line.id !== id)
    );
  }

  function addOption(product, type) {
    const linkedInstallations = installationLines.filter(
      (line) =>
        String(line.storeParentProductId || "") === String(product?.id || "")
    );
    const option = createStoreOption(
      product,
      type,
      linkedInstallations.length === 1 ? linkedInstallations[0] : null
    );
    ...
    focusStoreOptionTitle(option.id);
    return option;
  }
  ...
  function removeOption(id) {
    ...
    if (openNestedId === id) setOpenNestedId("");
  }

  function handleProductPriceEnter(event, product) {
    if (event.key !== "Enter") return;
    event.preventDefault();
    event.stopPropagation();

    if (!cleanText(product.storeUnitPriceInclVat)) return;

    const sectionId = String(product.storeSectionId || "");
    const groupProducts = productLines.filter(
      (item) => String(item.storeSectionId || "") === sectionId
    );
    const currentIndex = groupProducts.findIndex(
      (item) => item.id === product.id
    );
    const nextProduct = groupProducts[currentIndex + 1];

    if (nextProduct) {
      setOpenNestedId("");
      setOpenProductId(nextProduct.id);
      setActiveSectionId(nextProduct.storeSectionId || "");
      focusStoreProductTitle(nextProduct.id);
      return;
    }
  ...
  function handleOptionPriceEnter(event, option, product) {
    if (event.key !== "Enter") return;
    event.preventDefault();
    event.stopPropagation();

    if (!cleanText(option.storeUnitPriceInclVat) || !product?.id) return;

    const groupOptions = options.filter(
      (item) =>
        String(item.storeParentProductId || "") === String(product.id) ||
        String(item.replacementLineId || "") === String(product.id)
    );
    const currentIndex = groupOptions.findIndex((item) => item.id === option.id);
    const nextOption = groupOptions[currentIndex + 1];

    if (nextOption) {
      setOpenProductId(product.id);
      setOpenNestedId(nextOption.id);
      focusStoreOptionTitle(nextOption.id);
      return;
    }

    addOption(product, option.optionType || "addition");
  }
  ...
  async function saveTextTemplate() {
    ...
    try {
      ...
      setTemplateName("");
      setTemplateMessage(
        "✓ Tekst og vilkår er lagret som firmamal. Varer og priser er ikke med."
      );
    } catch (error) {
  ...
  function applySelectedTemplate() {
    ...
    [
      ...
      if (Object.prototype.hasOwnProperty.call(template.payload, field)) {
        updateOfferForm(field, template.payload[field] ?? "");
      }
    });
    setTemplateMessage(
      `✓ Malen «${template.name}» er lagt inn. Varer og priser er beholdt.`
    );
  }
  ...
  function buildDraftPreviewRequest() {
    const previewLines = lines.filter((line) => {
      if (isSectionLine(line)) return Boolean(sectionDescription(line));
      if (isProductLine(line)) return hasStoreProductDraftContent(line);
      return true;
    });
    const previewOptions = options.filter(hasStoreOptionDraftContent);

    const draftRequest = {
      ...selectedRequest,
      offerTitle: cleanText(offerForm.title),
      offerIntro: cleanText(offerForm.intro),
      offerLines: previewLines,
      offerOptions: previewOptions,
      offerReservations: cleanText(offerForm.reservations),
      offerIncluded: cleanText(offerForm.included),
      offerExcluded: cleanText(offerForm.excluded),
      ...
      <div className="store-draft-preview-shell">
        <div className="store-draft-preview-banner">
          <div>
            ...
            <span>
              – ikke publisert, ikke sendt. Kundevalgene vises, men kan ikke
              registreres her.
            </span>
          </div>
          <button
          ...
          >
            <ArrowLeft size={17} /> Tilbake til redigering
          </button>
        </div>
        <SalesCustomerView
          mode="customer-offer"
  ...
  const sectionsWithProducts = sections.map((section) => ({
    section,
    products: productLines.filter(
      (product) => String(product.storeSectionId || "") === String(section.id)
    ),
  }));
  const unsectionedProducts = productLines.filter(
    (product) =>
      !product.storeSectionId ||
      !sections.some(
        (section) => String(section.id) === String(product.storeSectionId)
      )
  );

  function renderProduct(product, globalIndex) {
    const productOpen = openProductId === product.id;
    const productInstallations = installationLines.filter(
      (line) =>
        String(line.storeParentProductId || "") === String(product.id)
    );
    const productOptions = options.filter(
      (option) =>
        String(option.storeParentProductId || "") === String(product.id) ||
        String(option.replacementLineId || "") === String(product.id)
    );
    const productSummary = `${product.description || "Uten varenavn"} · ${formatNok(
      storeGrossTotal(product)
    )} · ${
      productInstallations.length
        ? `${productInstallations.length} montering`
        : "ingen montering"
    } · ${
      productOptions.length ? `${productOptions.length} opsjon` : "ingen opsjon"
    }`;

    return (
      <article
        key={product.id}
        className={`store-product-container ${productOpen ? "is-open" : ""}`}
      >
        <CollapsibleHeader
          title={`Vare ${globalIndex + 1}`}
          summary={productSummary}
          open={productOpen}
          onToggle={() => {
            const nextOpen = productOpen ? "" : product.id;
            setOpenProductId(nextOpen);
            if (!productOpen) {
              setActiveSectionId(product.storeSectionId || "");
              focusStoreProductTitle(product.id);
            } else {
              setOpenNestedId("");
            }
          }}
        ...
        {productOpen ? (
          <div className="store-product-body">
            ...
            <ProductIdentityFields
              item={product}
              onPatch={(patch) => patchProduct(product.id, patch)}
              titleInputProps={{ "data-store-product-title": product.id }}
            />
            ...
            <PriceFields
              ...
              onPatch={(patch) => patchProduct(product.id, patch)}
              onPriceEnter={(event) =>
                handleProductPriceEnter(event, product)
              }
            />

            <DropZone
              item={product}
              onFiles={(files) => sendFiles("line", product.id, files)}
              onRemoveImage={() => removeOfferLineImage?.(product.id)}
              onRemoveAttachment={() => removeOfferLineAttachment?.(product.id)}
            />
            ...
            <div className="store-product-actions">
              <button
                type="button"
                className="sales-secondary-button"
                onClick={() => addInstallation(product)}
              >
                <Wrench size={16} /> Montering på denne varen
              </button>
              <button
                type="button"
                className="sales-secondary-button"
                onClick={() =>
                  setOptionPickerProductId((current) =>
                    current === product.id ? "" : product.id
                  )
                }
              >
                <Plus size={16} /> Opsjon på denne varen
              </button>
            </div>

            {optionPickerProductId === product.id ? (
              <div className="store-option-picker">
                <strong>Velg type opsjon</strong>
                <button
                  type="button"
                  onClick={() => addOption(product, "addition")}
                >
                  Tillegg / oppgradering
                </button>
                <button
                  type="button"
                  onClick={() => addOption(product, "alternative")}
                >
            ...
            {productInstallations.length || productOptions.length ? (
              <div className="store-product-children">
                {productInstallations.map((installation) => (
                  <InstallationEditor
                    key={installation.id}
                    item={installation}
                    open={openNestedId === installation.id}
                    onToggle={() =>
                      setOpenNestedId(
                        openNestedId === installation.id ? "" : installation.id
                      )
                    }
                    onPatch={(patch) =>
                      patchInstallation(installation.id, patch)
                    }
                    onRemove={() => removeInstallation(installation.id)}
                  />
                ))}
                ...
                {productOptions.map((option) => (
                  <OptionEditor
                    key={option.id}
                    option={option}
                    open={openNestedId === option.id}
                    onToggle={() => {
                      const nextOpen =
                        openNestedId === option.id ? "" : option.id;
                      setOpenNestedId(nextOpen);
                      if (nextOpen) focusStoreOptionTitle(option.id);
                    }}
                    product={product}
                    allProducts={productLines}
                    installations={installationLines}
                    ...
                    onPriceEnter={(event) =>
                      handleOptionPriceEnter(event, option, product)
                    }
                  />
                ))}
              </div>
  ...
  return (
    <div className="sales-app store-offer-builder-app store-grouped-builder">
      <style>{`
        ...
        .store-workspace{display:grid;gap:14px}.store-room{border:1px solid #cddfe5;border-radius:18px;background:#f8fcfd;overflow:hidden}.store-room.is-active{border-color:#16aeb9;box-shadow:0 0 0 3px rgba(22,174,185,.08)}.store-room-head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:13px 15px;background:#eef8fa;cursor:pointer}.store-room-title{display:grid;grid-template-columns:minmax(220px,.7fr) minmax(280px,1.3fr);gap:12px;min-width:0;flex:1}.store-room-title label{display:grid;gap:3px;min-width:0}.store-room-title label>span{font-size:11px;font-weight:850;letter-spacing:.03em;text-transform:uppercase;color:#66808a}.store-room-title input{font:inherit;font-size:18px;font-weight:900;border:1px solid transparent;background:transparent;min-width:180px;outline:none;padding:5px 7px;border-radius:8px}.store-room-title textarea{font:inherit;border:1px solid transparent;background:transparent;resize:vertical;color:#60727a;min-height:34px;outline:none;padding:5px 7px;border-radius:8px}.store-room-title input:focus,.store-room-title textarea:focus{background:#fff;border-color:#b9d8de}.store-room-actions{display:flex;gap:8px;align-items:center}.store-room-body{display:grid;gap:12px;padding:14px}.store-room-add{justify-self:start}
        ...
        @media(max-width:900px){.store-product-grid,.store-price-grid,.store-terms-grid,.store-follow-up-grid,.store-installation-custom-grid,.store-room-title{grid-template-columns:1fr 1fr}.store-product-link,.store-product-name{grid-column:1/-1}}
        @media(max-width:620px){.store-product-grid,.store-price-grid,.store-terms-grid,.store-follow-up-grid,.store-brand-grid,.store-template-grid,.store-option-top-grid,.store-installation-custom-grid,.store-room-title{grid-template-columns:1fr}.store-section-head,.store-summary,.store-draft-preview-banner{align-items:stretch;flex-direction:column}.store-line-total{align-items:flex-start}.store-alt-summary-delta{margin-left:0;text-align:left}.store-summary-actions{justify-content:stretch}.store-summary-actions button,.store-workbar button{width:100%}.store-workbar{position:sticky;bottom:6px}.store-room-head{align-items:flex-start;flex-direction:column}.store-room-actions{width:100%}}
      `}</style>
          ...
          >
            ...
            <section className="store-builder-section">
              ...
              <div className="store-template-grid">
                ...
                >
                  <Save size={17} /> Lagre tekst og vilkår som mal
                </button>
              </div>
              ...
              {templateMessage ? (
                <p className="store-template-message">{templateMessage}</p>
              ) : null}
              ...
              <div className="sales-form-grid" style={{ marginTop: 14 }}>
                ...
                <label className="sales-field sales-field-full">
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      updateOfferForm("intro", event.target.value)
                    }
                    rows={3}
                  />
                </label>
              ...
              <div className="store-terms-grid" style={{ marginTop: 12 }}>
                <label className="sales-field">
                  ...
                  <select
                    ...
                    onChange={(event) => {
                      ...
                      if (!value) updateOfferForm("paymentTerms", "");
                      else if (value === "custom")
                        updateOfferForm("paymentTerms", CUSTOM_CHOICE);
                      else updateOfferForm("paymentTerms", value);
                    }}
                    required
                  >
                    <option value="">Velg betalingsbetingelser</option>
                  ...
                  {paymentChoice === "custom" ? (
                    <input
                      style={{ marginTop: 8 }}
                      value={customFieldValue(offerForm.paymentTerms)}
                      onChange={(event) =>
                        updateOfferForm(
                          "paymentTerms",
                          event.target.value || CUSTOM_CHOICE
                        )
                      }
                      required
                    />
                ...
                <label className="sales-field">
                  ...
                  <select
                    ...
                    onChange={(event) => {
                      const value = event.target.value;
                      if (!value) updateOfferForm("validityDays", "");
                      else if (value === "custom")
                        updateOfferForm("validityDays", CUSTOM_CHOICE);
                      else updateOfferForm("validityDays", value);
                    }}
                    required
                  >
                    <option value="">Velg gyldighet</option>
                    {VALIDITY_CHOICES.map((item) => (
                      <option key={item} value={item}>
                        {item} dager
                      </option>
                    ))}
                  ...
                  {validityChoice === "custom" ? (
                    <input
                      style={{ marginTop: 8 }}
                      value={customFieldValue(offerForm.validityDays)}
                      onChange={(event) =>
                        updateOfferForm(
                          "validityDays",
                          event.target.value || CUSTOM_CHOICE
            ...
            <section className="store-builder-section">
              <div className="store-section-head">
                <div>
                  <h2>Automatisk oppfølging</h2>
                  <p>
                    Påminnelser stopper ved aksept, avvisning eller utløpt gyldighet.
                  </p>
                </div>
              </div>

              <label className="store-follow-up-toggle">
                <input
                  type="checkbox"
                  checked={followUp.enabled}
                  onChange={(event) =>
                    patchStoreMeta({ followUpEnabled: event.target.checked })
                  }
                />
                Send automatiske påminnelser dersom kunden ikke har svart
              </label>

              {followUp.enabled ? (
                <div className="store-follow-up-grid" style={{ marginTop: 12 }}>
                  <label className="sales-field">
                    ...
                    <input
                      ...
                      min="1"
                      max="90"
                      value={metaLine?.followUpFirstDays ?? followUp.firstDays}
                      onChange={(event) =>
                        patchStoreMeta({ followUpFirstDays: event.target.value })
                      }
                    />
                  </label>
                  <label className="sales-field">
                    ...
                    <input
                      ...
                      min="1"
                      max="90"
                      value={metaLine?.followUpRepeatDays ?? followUp.repeatDays}
                      onChange={(event) =>
                        patchStoreMeta({ followUpRepeatDays: event.target.value })
                      }
                    />
                  </label>
                  <label className="sales-field">
                    ...
                    <input
                      ...
                      max="10"
                      value={
                        metaLine?.followUpMaxReminders ?? followUp.maxReminders
                      }
                      onChange={(event) =>
                        patchStoreMeta({
                          followUpMaxReminders: event.target.value,
                        })
                      }
                    />
                  </label>
            ...
            <section className="store-builder-section">
              <div className="store-section-head">
                <div>
                  <h2>Varer og avsnitt</h2>
                  <p>
                    Bruk avsnitt som rom eller leveranseområder, f.eks. Varmepumpe,
                    Bad 1, Bad 2 eller Vaskerom. Montering og opsjoner ligger på
                    hovedvaren.
                  </p>
                </div>
              </div>
              ...
              <div className="store-workspace">
                {sectionsWithProducts.map(({ section, products }) => {
                  const roomOpen = openSections[section.id] !== false;
                  const roomActive = activeSectionId === section.id;

                  return (
                    <article
                      key={section.id}
                      className={`store-room ${roomActive ? "is-active" : ""}`}
                    >
                      <div
                        className="store-room-head"
                        onClick={() => {
                          setActiveSectionId(section.id);
                      ...
                      >
                        <div
                          className="store-room-title"
                          onClick={(event) => event.stopPropagation()}
                        >
                          <label>
                            <span>Avsnittsnavn</span>
                            <input
                              value={section.storeTextTitle || ""}
                              onFocus={() => setActiveSectionId(section.id)}
                              onChange={(event) =>
                                patchSection(section.id, {
                                  storeTextTitle: event.target.value,
                                })
                              }
                              placeholder="F.eks. Varmepumpe eller Bad 1"
                            />
                          </label>
                          <label>
                            <span>Kort tekst til kunden (valgfritt)</span>
                            <textarea
                              value={section.storeTextBody || ""}
                              onFocus={() => setActiveSectionId(section.id)}
                              onChange={(event) =>
                                patchSection(section.id, {
                                  storeTextBody: event.target.value,
                                })
                              }
                              rows={1}
                              placeholder="F.eks. leveranse og tilbehør til varmepumpe"
                            />
                          </label>
                        </div>
                      ...
                      {roomOpen ? (
                        <div className="store-room-body">
                          {products.length ? (
                            products.map((product) => {
                              const rendered = renderProduct(product, runningIndex);
                              runningIndex += 1;
                              return rendered;
                            })
                          ) : (
                            <div className="store-empty">
                              Ingen varer i dette avsnittet ennå.
                            </div>
                          )}

                          <button
                            type="button"
                            className="sales-secondary-button store-room-add"
                            onClick={() => addProduct(section.id)}
                          >
                            <Plus size={16} /> Legg til vare i{" "}
                            {section.storeTextTitle || "avsnittet"}
                          </button>
                        </div>
                      ) : null}
                    </article>
                ...
                {unsectionedProducts.length ? (
                  <article className="store-room">
                    <div
                      className="store-room-head"
                      onClick={() => setActiveSectionId("")}
                    >
                      <div className="store-room-title">
                        <strong>Uten avsnitt</strong>
                        <span>
                          Varer som ikke er plassert i et rom eller leveranseområde.
                        </span>
                      </div>
                    </div>
                    <div className="store-room-body">
                ...
                {!productLines.length && !sections.length ? (
                  <div className="store-empty">
                    Start med et avsnitt eller legg til første vare.
                  </div>
                ) : null}
              </div>
            </section>

            {standaloneInstallations.length ? (
              <section className="store-builder-section">
                <div className="store-section-head">
                  <div>
                    <h2>Kun montering</h2>
                    <p>Montering som ikke hører til en bestemt vare.</p>
                  </div>
                </div>
                <div className="store-standalone">
                  {standaloneInstallations.map((line) => (
                    <InstallationEditor
                      key={line.id}
                      item={line}
                      open={openNestedId === line.id}
                      onToggle={() =>
                        setOpenNestedId(
                          openNestedId === line.id ? "" : line.id
                        )
                      }
                      onPatch={(patch) => patchInstallation(line.id, patch)}
                      onRemove={() => removeInstallation(line.id)}
                    />
                  ))}
                </div>
              </section>
            ...
            {standaloneOptions.length ? (
              <section className="store-builder-section">
                <div className="store-section-head">
                  <div>
                    <h2>Andre opsjoner</h2>
                    <p>Eldre eller frittstående opsjoner uten kobling til en hovedvare.</p>
                  </div>
                </div>
                <div className="store-standalone">
            ...
            <div className="store-workbar">
              <button
                type="button"
                className="sales-primary-button"
                onClick={() => addProduct(activeSectionId)}
              >
                <Plus size={17} /> Legg til vare
              </button>
              <button
                type="button"
                className="sales-secondary-button"
                onClick={addSection}
              >
                <Plus size={17} /> Legg til avsnitt
              </button>
              <button
                type="button"
                className="sales-secondary-button"
                onClick={() => addInstallation(null)}
              >
                <Wrench size={17} /> Kun montering
              </button>
            </div>
            ...
            <div className="store-summary">
              ...
              <div className="store-summary-actions">
                <button
                  ...
                  disabled={!productLines.length && !installationLines.length}
                >
                  <Eye size={18} /> Forhåndsvis kundetilbud
                </button>
                <button
                  type="submit"
                  className="sales-primary-button"
