// Expo ProffDok – FASE 37A2 / FASE 37D1
// Egen varebygger for Butikktilbud. Brukeren arbeider med priser inkl. mva.
// FASE 37A2 gjør betalingsbetingelser og gyldighet obligatorisk og lar bruker
// velge versjonslåst automatisk oppfølgingsplan kun for Butikktilbud.
  ...
  return (
    <div className="store-product-grid">
      <label className="sales-field store-product-name">
        ...
        <input value={item[titleField] || ""} onChange={(event) => onPatch({ [titleField]: event.target.value })} placeholder="F.eks. servantskap 80 cm" />
      </label>
      <label className="sales-field">
        <span>NOBB-nr.</span>
        <div className="store-link-field">
          <input value={item.nobbNumber || ""} onChange={(event) => onPatch({ nobbNumber: event.target.value })} placeholder="NOBB-nummer" autoComplete="off" />
          {nobbUrl ? <a href={nobbUrl} target="_blank" rel="noreferrer" title="Åpne varen direkte i NOBB"><ExternalLink size={17} /></a> : null}
        </div>
      </label>
      ...
      <label className="sales-field store-product-link">
        ...
        <div className="store-link-field">
          <input value={item.productUrl || ""} onChange={(event) => onPatch({ productUrl: event.target.value, storeAutoProductUrl: false })} placeholder="NOBB eller leverandørens produktside" inputMode="url" />
          {item.productUrl ? <a href={item.productUrl} target="_blank" rel="noreferrer" title="Åpne produktlink"><Link2 size={17} /></a> : null}
        </div>
      </label>
    </div>
  );
...
function PriceFields({ item, onPatch, priceLabel = "Pris pr. enhet inkl. mva.", totalLabel = "Linjesum inkl. mva." }) {
  ...
  return (
    <div className="store-price-grid">
      ...
      <label className="sales-field"><span>{priceLabel}</span><input value={item.storeUnitPriceInclVat ?? ""} inputMode="decimal" onChange={(event) => onPatch({ storeUnitPriceInclVat: event.target.value })} placeholder="0" />{hasUnitPrice ? <small className="store-price-net">{formatNok(netFromGross(grossUnit))} eks. mva.</small> : null}</label>
...
function ProductCard({ item, index, dragProps, onPatch, onRemove, onFiles, onRemoveImage, onRemoveAttachment, renderCatalogLookup }) {
  return <article className="store-item-card" {...dragProps}><div className="store-item-heading"><div className="store-drag-handle"><GripVertical size={18} /></div><strong>Vare {index + 1}</strong><button type="button" className="store-icon-button" onClick={onRemove} aria-label="Slett vare"><Trash2 size={17} /></button></div>{renderCatalogLookup?.({ kind: "line", item, onPatch })}<ProductIdentityFields item={item} onPatch={onPatch} /><PriceFields item={item} onPatch={onPatch} /><DropZone item={item} onFiles={onFiles} onRemoveImage={onRemoveImage} onRemoveAttachment={onRemoveAttachment} /></article>;
}

function InstallationCard({ item, index, dragProps, onPatch, onRemove }) {
  return <article className="store-item-card store-installation-card" {...dragProps}><div className="store-item-heading"><div className="store-drag-handle"><GripVertical size={18} /></div><strong>Montering {index + 1}</strong><button type="button" className="store-icon-button" onClick={onRemove} aria-label="Slett montering"><Trash2 size={17} /></button></div><label className="sales-field"><span>Beskrivelse *</span><input value={item.description || ""} onChange={(event) => onPatch({ description: event.target.value })} placeholder="F.eks. Montering av ovennevnte baderomsmøbel" /></label><PriceFields item={item} onPatch={onPatch} priceLabel="Monteringspris inkl. mva." /></article>;
}

function DualAmount({ label, gross, delta = false }) {
  return <div><span>{label}</span><strong>{delta ? formatStoreDelta(gross) : formatNok(gross)} inkl. mva.</strong><small>{delta ? formatStoreDelta(netFromGross(gross)) : formatNok(netFromGross(gross))} eks. mva.</small></div>;
}

function AlternativeSummary({ option, lines }) {
  const breakdown = getStoreAlternativeBreakdown(option, lines);
  return <div className="store-alt-summary"><DualAmount label="Pris på alternativet" gross={breakdown.newItemTotal} />{breakdown.hasInstallationOverride ? <DualAmount label="Montering med alternativet" gross={breakdown.newInstallationTotal} /> : null}<div className="store-alt-summary-delta"><DualAmount label="Prisendring mot grunnpakken" gross={breakdown.totalDelta} delta /></div></div>;
}

function OptionCard({ option, index, productLines, installationLines, dragProps, onPatch, onReplacementChange, onInstallationChange, onRemove, onFiles, onRemoveImage, onRemoveAttachment, renderCatalogLookup }) {
  ...
  return (
    <article className="store-item-card store-option-card" {...dragProps}>
      <div className="store-item-heading"><div className="store-drag-handle"><GripVertical size={18} /></div><strong>Opsjon {index + 1}</strong><button type="button" className="store-icon-button" onClick={onRemove} aria-label="Slett opsjon"><Trash2 size={17} /></button></div>
      <div className="store-option-top-grid"><label className="sales-field"><span>Type opsjon</span><select value={option.optionType || "addition"} onChange={(event) => onPatch({ optionType: event.target.value })}><option value="addition">Tillegg / oppgradering</option><option value="alternative">Alternativ som erstatter</option></select></label>{alternative ? <label className="sales-field"><span>Erstatter *</span><select value={option.replacementLineId || ""} onChange={(event) => onReplacementChange(event.target.value)}><option value="">Velg vare eller montering</option>{productLines.map((target) => <option key={target.id} value={target.id}>Vare: {target.description || "Uten navn"}</option>)}{installationLines.map((target) => <option key={target.id} value={target.id}>Montering: {target.description || "Uten navn"}</option>)}</select></label> : null}</div>
      {!replacesInstallation ? renderCatalogLookup?.({ kind: "option", item: option, onPatch }) : null}
      {replacesInstallation ? <><label className="sales-field" style={{ marginTop: 12 }}><span>Beskrivelse *</span><input value={option.title || ""} onChange={(event) => onPatch({ title: event.target.value })} placeholder="Ny monteringsløsning" /></label><PriceFields item={option} onPatch={onPatch} priceLabel="Ny monteringspris inkl. mva." totalLabel="Ny monteringssum inkl. mva." /></> : <><ProductIdentityFields item={option} onPatch={onPatch} titleLabel={alternative ? "Alternativ vare *" : "Opsjonsvare *"} titleField="title" /><PriceFields item={option} onPatch={onPatch} priceLabel={alternative ? "Pris på alternativ vare inkl. mva." : "Pris pr. enhet inkl. mva."} totalLabel={alternative ? "Alternativ varepris inkl. mva." : "Opsjonssum inkl. mva."} /></>}
      {alternative && !replacesInstallation && installationLines.length ? <div className="store-installation-override"><div><strong>Montering med dette alternativet</strong><p>Valgfritt. Velg monteringspost hvis denne varen gir en annen monteringspris.</p></div><label className="sales-field"><span>Montering som påvirkes</span><select value={option.storeInstallationReplacementLineId || ""} onChange={(event) => onInstallationChange(event.target.value)}><option value="">Uendret montering</option>{installationLines.map((line) => <option key={line.id} value={line.id}>{line.description || "Montering"} – {formatNok(storeGrossTotal(line))} inkl. mva.</option>)}</select></label>{option.storeInstallationReplacementLineId ? <label className="sales-field"><span>Ny monteringspris inkl. mva.</span><input value={option.storeInstallationPriceInclVat ?? ""} onChange={(event) => onPatch({ storeInstallationPriceInclVat: event.target.value })} inputMode="decimal" placeholder="0" />{String(option.storeInstallationPriceInclVat ?? "").trim() ? <small className="store-price-net">{formatNok(netFromGross(installationGross))} eks. mva.</small> : null}</label> : null}</div> : null}
      {alternative ? <AlternativeSummary option={option} lines={[...productLines, ...installationLines]} /> : null}
      <label className="sales-field" style={{ marginTop: 12 }}><span>Beskrivelse / kundetekst</span><textarea value={option.description || ""} onChange={(event) => onPatch({ description: event.target.value, storeAutoDescription: false })} rows={2} placeholder="Valgfri forklaring til kunden" /></label>
...
export default function SalesStoreOfferBuilder(props) {
  ...
  useEffect(() => {
    ...
    const visibleBeforeInit = nextLines.some((line) => !line?.__storeOfferMeta) || options.length;
    if (!nextLines.some((line) => line?.__storeOfferMeta)) {
      nextLines.push(createStoreOfferMetaLine({ brandKey: DEFAULT_STORE_OFFER_BRAND.key, signatureName: selectedRequest?.responsible || selectedRequest?.projectResponsible || "", followUp: DEFAULT_STORE_FOLLOW_UP }));
    }
    ...
    if (!visibleBeforeInit && !selectedRequest?.sentOfferVersionId) {
      updateOfferForm("paymentTerms", "");
      updateOfferForm("validityDays", "");
    }
  ...
  function applyLineChanges(nextLines) { updateOfferForm("lines", nextLines); updateOfferForm("options", recalculateStoreOptions(options, nextLines)); }
  function replaceVisibleLines(nextVisibleLines, nextMeta = metaLine) { applyLineChanges([...nextVisibleLines, ...(nextMeta ? [nextMeta] : []), ...otherLines]); }
  function patchLine(id, patch) {
    const nextLines = lines.map((line) => {
      ...
      if (Object.prototype.hasOwnProperty.call(patch, "nobbNumber") && (!String(next.productUrl || "").trim() || next.storeAutoProductUrl)) { next.productUrl = buildNobbItemUrl(patch.nobbNumber); next.storeAutoProductUrl = Boolean(next.productUrl); }
  ...
  function patchOption(id, patch) {
    const nextOptions = options.map((option) => {
      ...
      if (Object.prototype.hasOwnProperty.call(patch, "supplierProductNumber")) next.internalProductNumber = patch.supplierProductNumber;
      if (patch.optionType === "addition") next = { ...next, mainPostId: PRODUCT_POST.id, mainPostTitle: PRODUCT_POST.title, replacementLineId: "", replacementLineDescription: "", storeInstallationReplacementLineId: "", storeInstallationPriceInclVat: "" };
  ...
  function changeReplacement(optionId, replacementLineId) {
    ...
    const patch = { replacementLineId, replacementLineDescription: target?.description || "", mainPostId: target?.mainPostId || PRODUCT_POST.id, mainPostTitle: target?.mainPostTitle || PRODUCT_POST.title };
    if (target?.mainPostId === PRODUCT_POST.id && installationLines.length === 1 && !current.storeInstallationReplacementLineId) { patch.storeInstallationReplacementLineId = installationLines[0].id; patch.storeInstallationPriceInclVat = storeNumber(storeGrossTotal(installationLines[0])); }
    if (target?.mainPostId === INSTALLATION_POST.id) { patch.storeInstallationReplacementLineId = ""; patch.storeInstallationPriceInclVat = ""; }
    patchOption(optionId, patch);
  }
  function changeOptionInstallation(optionId, installationLineId) { const line = installationLines.find((item) => item.id === installationLineId); patchOption(optionId, { storeInstallationReplacementLineId: installationLineId, storeInstallationPriceInclVat: line ? storeNumber(storeGrossTotal(line)) : "" }); }
  ...
  function reorderLines(section, fromId, toId) { const reordered = section === "product" ? [...moveItem(productLines, fromId, toId), ...installationLines] : [...productLines, ...moveItem(installationLines, fromId, toId)]; replaceVisibleLines(reordered); }
  ...
  function draggableProps(kind, id, section = "") { return { draggable: true, onDragStart: (event) => { if (event.target?.closest?.("input, textarea, select, button, a")) { event.preventDefault(); return; } setDragged({ kind, id, section }); event.dataTransfer.effectAllowed = "move"; }, onDragOver: (event) => { if (!dragged || dragged.kind !== kind) return; if (kind === "line" && dragged.section !== section) return; event.preventDefault(); }, onDrop: (event) => { if (event.dataTransfer?.files?.length) return; if (!dragged || dragged.kind !== kind || dragged.id === id) return; if (kind === "line" && dragged.section !== section) return; event.preventDefault(); if (kind === "option") reorderOptions(dragged.id, id); else reorderLines(section, dragged.id, id); setDragged(null); }, onDragEnd: () => setDragged(null) }; }
  ...
  function submitStoreOffer(event) {
    ...
    const rawRepeat = Number.parseInt(String(metaLine?.followUpRepeatDays ?? followUp.repeatDays), 10);
    const rawMax = Number.parseInt(String(metaLine?.followUpMaxReminders ?? followUp.maxReminders), 10);
    if (followUp.enabled && (!Number.isFinite(rawFirst) || rawFirst < 1 || rawFirst > 90 || !Number.isFinite(rawRepeat) || rawRepeat < 1 || rawRepeat > 90 || !Number.isFinite(rawMax) || rawMax < 1 || rawMax > 10)) { event.preventDefault(); alert("Kontroller oppfølgingsplanen: dager må være 1–90 og maks antall påminnelser 1–10."); return; }
  ...
  if (previewOpen) {
    ...
    return <div className="store-draft-preview-shell"><style>{`.store-draft-preview-shell .sales-customer-accept-form{display:none!important}.store-draft-preview-banner{position:sticky;top:0;z-index:25000;display:flex;align-items:center;justify-content:space-between;gap:14px;padding:12px 18px;background:#fff8d9;border-bottom:1px solid #ead78d;color:#493d11;font-weight:800}.store-draft-preview-banner span{font-weight:650}.store-draft-preview-banner button{white-space:nowrap}@media(max-width:620px){.store-draft-preview-banner{align-items:stretch;flex-direction:column}}`}</style><div className="store-draft-preview-banner"><div><strong>FORHÅNDSVISNING</strong> <span>– ikke publisert, ikke sendt og kan ikke aksepteres eller avvises av kunden.</span></div><button type="button" className="sales-secondary-button" onClick={() => setPreviewOpen(false)}><ArrowLeft size={17}/> Tilbake til redigering</button></div><SalesCustomerView mode="customer-offer" selectedRequest={previewRequest} companyProfile={{}} acceptanceForm={{ confirmed: false, name: "", selectedOptionIds: [] }} setAcceptanceForm={() => {}} toggleAcceptedOption={() => {}} handleAcceptOffer={(event) => event?.preventDefault?.()} onBack={() => setPreviewOpen(false)} /></div>;
  }
  ...
  return (
    <div className="sales-app store-offer-builder-app">
      <style>{`
        ...
        @media(max-width:900px){.store-product-grid,.store-price-grid,.store-terms-grid,.store-follow-up-grid,.store-installation-override{grid-template-columns:1fr 1fr}.store-product-link,.store-product-name,.store-installation-override>div:first-child{grid-column:1/-1}}
        @media(max-width:620px){.store-product-grid,.store-price-grid,.store-terms-grid,.store-follow-up-grid,.store-brand-grid,.store-template-grid,.store-option-top-grid,.store-installation-override{grid-template-columns:1fr}.store-section-head,.store-summary{align-items:stretch;flex-direction:column}.store-line-total{align-items:flex-start}.store-alt-summary-delta{margin-left:0;text-align:left}.store-summary-actions{justify-content:stretch}.store-summary-actions button{width:100%}}
      `}</style>
        ...
        <main className="sales-main">
          ...
          <form className="sales-form-panel" onSubmit={submitStoreOffer}>
            ...
            <section className="store-builder-section">
              <div className="store-section-head"><div><h2>Tilbudstekst og vilkår</h2><p>Betalingsbetingelser og gyldighet er obligatoriske i Butikktilbud. Kunden skal kunne se tydelig hva som gjelder før aksept eller avvisning.</p></div></div>
              <div className="store-template-grid"><label className="sales-field"><span>Malnavn</span><input value={templateName} onChange={(event) => setTemplateName(event.target.value)} placeholder="F.eks. Standard butikktilbud" /></label><button type="button" className="sales-secondary-button" disabled={!templateName.trim() || templateBusy} onClick={saveTextTemplate}><Save size={17} /> Lagre tekst og vilkår som mal</button></div>
            ...
            <section className="store-builder-section">
              <div className="store-section-head"><div><h2>Automatisk oppfølging</h2><p>Gjelder kun dette Butikktilbudet og låses med publisert versjon. Påminnelser stopper ved aksept, avvisning eller utløpt gyldighet.</p></div></div>
              <label className="store-follow-up-toggle"><input type="checkbox" checked={followUp.enabled} onChange={(event) => patchStoreMeta({ followUpEnabled: event.target.checked })} /> Send automatiske påminnelser dersom kunden ikke har svart</label>
              {followUp.enabled ? <div className="store-follow-up-grid" style={{marginTop:12}}><label className="sales-field"><span>Første påminnelse etter *</span><input type="number" min="1" max="90" value={metaLine?.followUpFirstDays ?? followUp.firstDays} onChange={(event) => patchStoreMeta({ followUpFirstDays: event.target.value })} /><small>Dager etter at tilbudet er sendt.</small></label><label className="sales-field"><span>Gjenta hver *</span><input type="number" min="1" max="90" value={metaLine?.followUpRepeatDays ?? followUp.repeatDays} onChange={(event) => patchStoreMeta({ followUpRepeatDays: event.target.value })} /><small>Dager mellom automatiske påminnelser.</small></label><label className="sales-field"><span>Maks antall påminnelser *</span><input type="number" min="1" max="10" value={metaLine?.followUpMaxReminders ?? followUp.maxReminders} onChange={(event) => patchStoreMeta({ followUpMaxReminders: event.target.value })} /><small>Deretter må saken vurderes manuelt.</small></label></div> : <p className="sales-subtitle" style={{margin:"12px 0 0"}}>Ingen automatiske påminnelser sendes. Saken kan fortsatt følges opp manuelt i Sales.</p>}
            </section>

            <section className="store-builder-section"><div className="store-section-head"><div><h2>Varer</h2><p>Prisene registreres inkl. mva. Saksbehandler ser automatisk tilsvarende pris eks. mva. NOBB-nr. gir direkte NOBB-link når egen produktlink ikke er satt.</p></div><button className="sales-primary-button" type="button" onClick={addProduct}><Plus size={18} /> Legg til vare</button></div><div className="store-item-list">{productLines.length ? productLines.map((item,index)=><ProductCard key={item.id} item={item} index={index} dragProps={draggableProps("line",item.id,"product")} onPatch={(patch)=>patchLine(item.id,patch)} onRemove={()=>removeLine(item.id)} onFiles={(files)=>sendFiles("line",item.id,files)} onRemoveImage={()=>removeOfferLineImage?.(item.id)} onRemoveAttachment={()=>removeOfferLineAttachment?.(item.id)} renderCatalogLookup={renderCatalogLookup} />) : <div className="store-empty"><ImagePlus size={24}/><br/>Ingen varer ennå. Legg til første vare.</div>}</div></section>
