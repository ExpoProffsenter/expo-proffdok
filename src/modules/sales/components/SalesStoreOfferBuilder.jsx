...
function ProductCard({ item, index, dragProps, onPatch, onRemove, onFiles, onRemoveImage, onRemoveAttachment }) {
  return <article className="store-item-card" {...dragProps}><div className="store-item-heading"><div className="store-drag-handle"><GripVertical size={18} /></div><strong>Vare {index + 1}</strong><button type="button" className="store-icon-button" onClick={onRemove} aria-label="Slett vare"><Trash2 size={17} /></button></div><ProductIdentityFields item={item} onPatch={onPatch} /><PriceFields item={item} onPatch={onPatch} /><DropZone item={item} onFiles={onFiles} onRemoveImage={onRemoveImage} onRemoveAttachment={onRemoveAttachment} /></article>;
}
...
export default function SalesStoreOfferBuilder(props) {
  const { selectedRequest, offerForm, offerDraftSaveStatus, onBack, handleSaveOffer, updateOfferForm, handleOfferLineFile, removeOfferLineImage, removeOfferLineAttachment, handleOfferOptionFile, removeOfferOptionImage, removeOfferOptionAttachment } = props;
  ...
  useEffect(() => {
    ...
    // Nye Butikktilbud skal kreve et bevisst valg. Generiske standardverdier fra
    // ordinært Sales-skjema skal ikke silently bli Butikktilbud-vilkår.
