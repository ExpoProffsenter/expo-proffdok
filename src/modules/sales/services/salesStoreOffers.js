...
// Enkel ordre versjonslåses i samme metadata uten ny parallell tilbudsmotor.
...
export const STORE_OFFER_BRANDS = [
  { key: "bademiljo-expo", label: "Bademiljø Expo", logoUrl: "/brands/bademiljo-expo-ringside.png" },
  { key: "ringside-rorleggerbedrift", label: "Ringside Rørleggerbedrift", logoUrl: "/brands/ringside-rorleggerbedrift.png" },
];
...
export function isRingsideStoreOfferProfile(profile = {}) {
  const orgNumber = profile?.org_number || profile?.orgNumber || profile?.company_org_number || profile?.companyOrgNumber || "";
  return digitsOnly(orgNumber) === RINGSIDE_STORE_OFFER_ORG_NUMBER;
}

export function markStoreOfferLaunch() {
  if (typeof window === "undefined") return;
  try { window.sessionStorage?.setItem(STORE_OFFER_SESSION_KEY, "1"); } catch {}
}
export function readStoreOfferLaunch() {
  if (typeof window === "undefined") return false;
  try { return window.sessionStorage?.getItem(STORE_OFFER_SESSION_KEY) === "1"; } catch { return false; }
}
export function clearStoreOfferLaunch() {
  if (typeof window === "undefined") return;
  try { window.sessionStorage?.removeItem(STORE_OFFER_SESSION_KEY); } catch {}
}

function findStoreMeta(request = {}) {
...
export function isStoreOfferRequest(request = {}) {
  if (findStoreMeta(request)) return true;
  return Boolean(request?.directOffer && String(request?.source || "").trim() === STORE_OFFER_SOURCE);
}

export function isSimpleOrderRequest(request = {}) {
  const meta = findStoreMeta(request);
  return Boolean(meta?.simpleOrder === true || meta?.offerKind === "simple-order-v1");
}

export function getStoreOfferBrand(brandKey = "") {
  const brand = STORE_OFFER_BRANDS.find((item) => item.key === brandKey) || DEFAULT_STORE_OFFER_BRAND;
  return { ...brand, logoUrl: absoluteBrandLogoUrl(brand.logoUrl) };
}
...
export function getStoreFollowUpConfig(meta = {}) {
  ...
  return {
    enabled,
    firstDays: boundedInteger(meta?.followUpFirstDays, DEFAULT_STORE_FOLLOW_UP.firstDays, 1, 90),
    repeatDays: boundedInteger(meta?.followUpRepeatDays, DEFAULT_STORE_FOLLOW_UP.repeatDays, 1, 90),
    maxReminders: boundedInteger(meta?.followUpMaxReminders, DEFAULT_STORE_FOLLOW_UP.maxReminders, 1, 10),
  };
}

export function createStoreOfferMetaLine({ brandKey = DEFAULT_STORE_OFFER_BRAND.key, signatureName = "", followUp = DEFAULT_STORE_FOLLOW_UP } = {}) {
