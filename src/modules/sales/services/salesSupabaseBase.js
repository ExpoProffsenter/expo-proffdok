// Expo ProffDok – FASE 30C2 / FASE 23D / FASE 28A1 / FASE 29B2 / FASE 29B3 / FASE 29B4 / FASE 29B5
// Samler Supabase-kall for Befaring / Tilbud / Aksept.
// FASE 30C2 husker sist hentede/lagrede salgssak og sender bare rader som faktisk
// har endret innhold. Dette hindrer at én tilbuds-autosave skriver hele sakslisten på nytt.
// FASE 30C2 registrerer også en uforanderlig lokal serverbaseline kun etter ekte
// Supabase-henting eller bekreftet lagring. Offline-kladd kan aldri flytte denne baselinen.
// FASE 30C2 eksponerer ekte lastestatus for salgssaker, slik at UI ikke viser 0
// mens Supabase fortsatt arbeider eller har feilet.
...
const salesRequestSignatures = new Map();
...
function setSalesRequestLoadState(nextState = {}) {
  ...
  salesRequestLoadListeners.forEach((listener) => {
    try {
      ...
      // Lastestatus er kun presentasjonsstøtte og skal aldri stoppe datalaget.
    }
  });
}
...
function salesRequestSignatureKey(companyId = "", requestRef = "") {
  return `${String(companyId || "").trim()}:${String(requestRef || "").trim()}`;
}

function createSalesRequestSignature(row = {}) {
  try {
    return JSON.stringify({
      status: row.status || "Forespørsel",
...
function rememberSalesRequestRows(companyId, rows = []) {
  (Array.isArray(rows) ? rows : []).forEach((row) => {
    const requestRef = row?.request_ref;
    if (!requestRef) return;
    salesRequestSignatures.set(
      salesRequestSignatureKey(companyId, requestRef),
      createSalesRequestSignature(row)
    );
  });
}
...
function getServerOfferMetrics(payload = {}) {
  const lines = Array.isArray(payload?.offerLines) ? payload.offerLines : [];
  const options = Array.isArray(payload?.offerOptions) ? payload.offerOptions : [];

  return {
    meaningfulLines: lines.filter(
      (line) =>
        String(line?.description || "").trim() ||
        String(line?.amount ?? "").trim() ||
        String(line?.internalProductNumber || "").trim()
    ).length,
    meaningfulOptions: options.filter(
      (option) =>
...
async function rememberConfirmedServerOfferBaselines(
  client,
  rows = [],
  fallbackCompanyId = ""
) {
  ...
  (Array.isArray(rows) ? rows : []).forEach((row) => {
    ...
    if (!companyId || !requestRef) return;

    const payload = row?.payload || {};
    const metrics = getServerOfferMetrics(payload);
    const offerDraftSavedAt = String(
      payload?.offerDraftSavedAt || row?.updated_at || observedAt
    ).trim();
    ...
    try {
      storage.setItem(
        ...
        JSON.stringify({
          ...
          offerDraftSavedAt,
          observedAt,
          meaningfulLines: metrics.meaningfulLines,
          meaningfulOptions: metrics.meaningfulOptions,
        })
      );
    } catch {
      // Baseline er et ekstra sikkerhetsnett. Supabase-resultatet skal ikke feile
      // dersom nettleseren nekter lokal lagring.
    }
  });
}
...
export async function fetchSalesRequests(client, companyId) {
  ...
  try {
    const result = await client
      .from("sales_requests")
      .select("request_ref,payload,status,archived_at,updated_at")
    ...
    rememberSalesRequestRows(companyId, result?.data || []);
    await rememberConfirmedServerOfferBaselines(
      client,
      result?.data || [],
      companyId
    );
...
export async function upsertSalesRequests(client, rows) {
  const safeRows = (Array.isArray(rows) ? rows : []).filter(
    (row) => row?.company_id && row?.request_ref
  );
  const changedRows = safeRows.filter((row) => {
    const key = salesRequestSignatureKey(row.company_id, row.request_ref);
    return salesRequestSignatures.get(key) !== createSalesRequestSignature(row);
  });
  ...
  const result = await client
    .from("sales_requests")
    .upsert(changedRows, { onConflict: "company_id,request_ref" });

  if (!result?.error) {
    changedRows.forEach((row) => {
      salesRequestSignatures.set(
        salesRequestSignatureKey(row.company_id, row.request_ref),
        createSalesRequestSignature(row)
      );
    });
    await rememberConfirmedServerOfferBaselines(client, changedRows);
  }
