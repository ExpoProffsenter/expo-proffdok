...
function App(){
 ...
 const addFiles=fl=>setFiles(p=>[...p,...Array.from(fl||[]).map(f=>({id:crypto.randid:uid()omUUID(),name:f.name,url:URL.createObjectURL(f),by:user.name||'Ukjent',created:new Date().toLocaleString('no-NO')}))]);
 ...
 {tab==='installasjoner'&&<Section title="Rør, deler, utstyr"><button onClick={()=>setInst([...inst,{id:uid(),category:'Rør',name:'',qty:'',supplier:'',desc:'',by:user.name||'Ukjent',created:new Date().toLocaleString('no-NO')}])}><Plus size={18}/> Legg til post</button>{inst.map(x=><div className="item" key={x.id}><Grid><Select label="Kategori" value={x.category} options={installCats} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,category:v}:i))}/><Input label="Navn/produkt" value={x.name} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,name:v}:i))}/><Input label="Antall/mengde" value={x.qty} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,qty:v}:i))}/><Input label="Leverandør" value={x.supplier} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,supplier:v}:i))}/><Textarea label="Beskrivelse/plassering" value={x.desc} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,desc:v}:i))}/></Grid><small>Lagt inn av {x.by} · {x.created}</small><button className="secondary" onClick={()=>setInst(inst.filter(i=>i.id!==x.id))}>Fjern</button></div>)}</Section>}
