...
const installCats=['Rør','Sluk','Deler','Utstyr','Varmekabler','Ventilasjon','Annet'];
...
function App(){
 ...
 const [project,setProject]=useState({responsible:'',projectName:'',address:'',customer:'',date:new Date().toISOString().slice(0,10),notes:''});
 const [checked,setChecked]=useState({}); const [other,setOther]=useState({}); const [surf,setSurf]=useState({});
 ...
 const addPhoto=(cat,fl)=>setPhotos(p=>[...p,...Array.from(fl||[]).map(f=>({id:crypto.randomUUID(),cat,url:URL.createObjectURL(f),comment:'',created:new Date().toLocaleString('no-NO')}))]);
 const addFiles=fl=>setFiles(p=>[...p,...Array.from(fl||[]).map(f=>({id:crypto.randomUUID(),name:f.name,url:URL.createObjectURL(f),by:user.name||'Ukjent',created:new Date().toLocaleString('no-NO')}))]);
 ...
 {tab==='prosjekt'&&<Section title="Prosjektinformasjon" icon={<ClipboardCheck/>}><Grid><Input label="Prosjektansvarlig" value={project.responsible} onChange={v=>setProject({...project,responsible:v})}/><Input label="Dato" type="date" value={project.date} onChange={v=>setProject({...project,date:v})}/><Input label="Navn på prosjekt" value={project.projectName} onChange={v=>setProject({...project,projectName:v})}/><Input label="Adresse" value={project.address} onChange={v=>setProject({...project,address:v})}/><Input label="Kunde" value={project.customer} onChange={v=>setProject({...project,customer:v})}/><Textarea label="Notater" value={project.notes} onChange={v=>setProject({...project,notes:v})}/></Grid></Section>}
 {tab==='firma'&&<Section title="Firma / white-label" icon={<Building2/>}><p className="note">Kundens logo og firmainfo vises i appen og rapporten. Uten kundelogo brukes Expo Proffsenter som standard.</p><div className="two"><div className="logoBox"><Brand logo={company.logoUrl} name={name}/><label className="upload"><Plus size={18}/> Last opp kundelogo<input type="file" accept="image/*" onChange={e=>setCompany({...company,logoUrl:URL.createObjectURL(e.target.files[0])})}/></label>{company.logoUrl&&<button className="secondary" onClick={()=>setCompany({...company,logoUrl:''})}>Fjern logo</button>}</div><Grid><Input label="Firmanavn" value={company.companyName} onChange={v=>setCompany({...company,companyName:v})}/><Input label="Org.nr" value={company.orgNumber} onChange={v=>setCompany({...company,orgNumber:v})}/><Input label="Adresse" value={company.address} onChange={v=>setCompany({...company,address:v})}/><Input label="Telefon" value={company.phone} onChange={v=>setCompany({...company,phone:v})}/><Input label="E-post" value={company.email} onChange={v=>setCompany({...company,email:v})}/><Input label="Hjemmeside" value={company.website} onChange={v=>setCompany({...company,website:v})}/></Grid></div></Section>}
 {tab==='innlogging'&&<Section title="Innlogging og brukerprofil" icon={<BadgeCheck/>}><p className="note">Demo av brukerprofil. I ferdig løsning kobles dette til ekte innlogging og database.</p><Grid><Input label="Navn" value={user.name} onChange={v=>setUser({...user,name:v})}/><Input label="E-post" value={user.email} onChange={v=>setUser({...user,email:v})}/><Select label="Rolle" value={user.role} options={roles} onChange={v=>setUser({...user,role:v})}/></Grid></Section>}
 ...
 {tab==='tilgang'&&<Section title="Del tilgang til prosjekt"><button onClick={()=>setAccess([...access,{id:crypto.randomUUID(),name:'',email:'',role:'Underleverandør'}])}><Plus size={18}/> Legg til tilgang</button>{access.map(a=><div className="row" key={a.id}><Input label="Navn" value={a.name} onChange={v=>setAccess(access.map(x=>x.id===a.id?{...x,name:v}:x))}/><Input label="E-post" value={a.email} onChange={v=>setAccess(access.map(x=>x.id===a.id?{...x,email:v}:x))}/><Select label="Rolle" value={a.role} options={roles} onChange={v=>setAccess(access.map(x=>x.id===a.id?{...x,role:v}:x))}/><button className="secondary" onClick={()=>setAccess(access.filter(x=>x.id!==a.id))}>Fjern</button></div>)}</Section>}
 {tab==='installasjoner'&&<Section title="Rør, deler, utstyr"><button onClick={()=>setInst([...inst,{id:crypto.randomUUID(),category:'Rør',name:'',qty:'',supplier:'',desc:'',by:user.name||'Ukjent',created:new Date().toLocaleString('no-NO')}])}><Plus size={18}/> Legg til post</button>{inst.map(x=><div className="item" key={x.id}><Grid><Select label="Kategori" value={x.category} options={installCats} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,category:v}:i))}/><Input label="Navn/produkt" value={x.name} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,name:v}:i))}/><Input label="Antall/mengde" value={x.qty} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,qty:v}:i))}/><Input label="Leverandør" value={x.supplier} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,supplier:v}:i))}/><Textarea label="Beskrivelse/plassering" value={x.desc} onChange={v=>setInst(inst.map(i=>i.id===x.id?{...i,desc:v}:i))}/></Grid><small>Lagt inn av {x.by} · {x.created}</small><button className="secondary" onClick={()=>setInst(inst.filter(i=>i.id!==x.id))}>Fjern</button></div>)}</Section>}
...
function Brand({logo,name}) {
  return (
    <div style={{ width: "260px", height: "80px", overflow: "hidden", display: "flex", alignItems: "center" }}>
      <img
        src={logo ? logo : "/expo-logo.png"}
        alt={name || "Expo Proffsenter"}
...
function Report({company,name,project,selected,other,surf,photos,access,inst,files}){return <div className="report"><section><div className="reportTop"><Brand logo={company.logoUrl} name={name}/><div><h2>{name}</h2>{company.address&&<p>{company.address}</p>}{company.orgNumber&&<p>Org.nr: {company.orgNumber}</p>}{company.phone&&<p>{company.phone}</p>}{company.email&&<p>{company.email}</p>}{company.website&&<p>{company.website}</p>}</div></div><h2>FDV-rapport / Prosjektdokumentasjon</h2><Grid>{Object.entries(project).map(([k,v])=><div className="out" key={k}><b>{k}</b><p>{v||'Ikke fylt ut'}</p></div>)}</Grid></section><section><h2>Produkter</h2>{selected.map(p=><p key={p.item}><b>{p.section}:</b> {p.item}</p>)}{Object.entries(other).filter(([,v])=>v).map(([k,v])=><p key={k}><b>{k} annet:</b> {v}</p>)}</section><section><h2>Overflater</h2>{Object.entries(surf).filter(([,v])=>v).map(([k,v])=><p key={k}><b>{k}:</b> {v}</p>)}</section><section><h2>Bildedokumentasjon</h2><div className="photos reportPhotos">{photos.map(p=><div className="photo" key={p.id}><img src={p.url}/><b>{p.cat}</b><p>{p.comment}</p></div>)}</div></section><section><h2>Rør, deler og utstyr</h2>{inst.map(i=><p key={i.id}><b>{i.category}:</b> {i.name} {i.qty&&`· ${i.qty}`} {i.desc&&` — ${i.desc}`}</p>)}</section><section><h2>Sjekklister og vedlegg</h2>{files.map(f=><p key={f.id}>{f.name}</p>)}</section><section><h2>Prosjekttilgang</h2>{access.map(a=><p key={a.id}>{a.name||a.email} — {a.role}</p>)}</section><footer>Levert av Expo Proffsenter</footer></div>}
