-- FASE 45B – preserve existing Production semantics for internal sensitive access.
-- Required because an earlier Sandbox-only restore used SQL NULL comparison for system_role.
-- This migration is idempotent and deliberately matches current Production behavior.
...
begin
  ...
  if coalesce(p_view_internal_net_prices,false) and not v_target_is_systemadmin then
    ...
    where user_id=target_user_id and feature_key='view_internal_net_prices';
  end if;
