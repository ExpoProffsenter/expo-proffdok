-- Expo ProffDok – FASE 42C
-- Egen teller for gamle Cordel-leverandørfiler med ÅVP-prefiks.
-- Utvider klargjøring av katalog uten å endre aktiv katalog eller historiske tilbud.

alter table public.internal_store_catalog_imports
  add column if not exists skipped_avp_supplier_rows bigint not null default 0;

create or replace function public.prepare_internal_store_catalog_activation_v3(
  p_import_id uuid,
  p_total_rows bigint,
  ...
  p_skipped_avp_supplier_rows bigint default 0
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  v_duplicate_rows := greatest(
    coalesce(p_total_rows,0)
      ...
      - greatest(coalesce(p_skipped_avp_supplier_rows,0),0)
  ...
  set status = 'ready',
      ...
      skipped_avp_supplier_rows = greatest(coalesce(p_skipped_avp_supplier_rows,0),0),
...
begin
  ...
  return jsonb_build_object(
    ...
    'skipped_avp_supplier_rows', v_row.skipped_avp_supplier_rows,
...
revoke all on function public.prepare_internal_store_catalog_activation_v3(uuid,bigint,bigint,bigint,bigint,bigint,bigint) from public, anon;
grant execute on function public.prepare_internal_store_catalog_activation_v3(uuid,bigint,bigint,bigint,bigint,bigint,bigint) to authenticated;
