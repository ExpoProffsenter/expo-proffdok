-- FASE 45B – Produkter fra akseptert tilbud følger automatisk Enkel ordre.
-- Ingen ny ordretabell. Vi beriker prosjektmotorens eksisterende manualProducts-format
-- med produktidentitet fra den LÅSTE aksepterte tilbudsversjonen.
-- Pris/nto-pris kopieres ikke til produktlisten.
...
begin
  if v_workflow<>'simple_order' and coalesce((new.data->'project'->>'simpleOrder')::boolean,false)=false then
    return new;
  end if;
  ...
  where o.request_ref=v_request_ref
    ...
    and (
      new.company_scope_id is null
      or o.company_id=new.company_scope_id
    )
  ...
  v_selected:=case when jsonb_typeof(v_accepted->'selected_options')='array'
    then v_accepted->'selected_options' else '[]'::jsonb end;

  with accepted_items as (
    select elem, false as is_option
    from jsonb_array_elements(
      case when jsonb_typeof(v_snapshot->'lines')='array' then v_snapshot->'lines' else '[]'::jsonb end
    ) t(elem)
    ...
    from jsonb_array_elements(
      case when jsonb_typeof(v_snapshot->'options')='array' then v_snapshot->'options' else '[]'::jsonb end
    ) o(opt)
    where exists(
  ...
  select coalesce(jsonb_agg(
    ...
    )) order by is_option,coalesce(elem->>'description',elem->>'title','')
  ),'[]'::jsonb)
  into v_products
  from accepted_items;

  if jsonb_array_length(v_products)>0 then
    -- Bevar eventuelle manuelt registrerte produktgrupper. Bare vår dedikerte gruppe seedes.
  ...
  -- Sørg samtidig for at den låste tilbudskilden finnes i prosjektdata for historikk/fremdrift.
  new.data:=jsonb_set(
    new.data,
    '{tilbud,acceptedOfferSnapshot}',
    jsonb_build_object(
  ...
  new.data:=jsonb_set(new.data,'{tilbud,enabled}','true'::jsonb,true);
...
drop trigger if exists trg_fase45b_seed_simple_order_accepted_products on public.projects;
create trigger trg_fase45b_seed_simple_order_accepted_products
before insert on public.projects
for each row execute function public.fase45b_seed_simple_order_accepted_products();
